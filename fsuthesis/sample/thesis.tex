...
\usepackage{iftex}
\ifPDFTeX   % We execute this part (up to \else) for pdfTeX
...
\else    % Otherwise we'll run this part if not running pdfTeX
  \ifXeTeX
    ...
    \usepackage[colorlinks=false,bookmarks=true,pdfborder={0 0 0},
      linkcolor=myblue,urlcolor=myred,citecolor=mygreen,
      breaklinks=true,bookmarksnumbered=true]{hyperref}
    % Add fontspec and listings packages for XeTeX
    \usepackage{fontspec}
    \usepackage{listings}
  \else
...
\degreeyear{2026}                     %% Graduation Year
\defensedate{April 1, 2026}           %% Date of Defense
...
\committeeperson{Aussem Pauers}{Committee Member}
...
%%% If one has computer code that one wants to include the thesis
%%% using the 'listings' package, the following chapter contains a
%%% test portion loading some additional local fonts. This will
%%% require using XeTeX or LuaTeX engines to access the local fonts
%%% rather than pdfTeX. Refer to the codelisting.tex file for
