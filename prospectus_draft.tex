...
\begin{figure}[h!]
  ...
  \caption{Figure 4.1: Cartoon showing logic setup used for the CAEN digitzers. The RF signal was recorded in the 1730 board.}
...
\begin{figure}[h!]
  ...
  \caption{Figure 4.3: ToF spectrum for the $^{12}C(^3He,n)^{14}O$ reaction observed by a CATRiNA detector at 133.5 degrees.}
  \label{fig:14O ToF}
\end{figure}
...
\begin{figure}[h!]
  ...
  \caption{Figure 4.4: ToFg spectrum.}
...
\begin{figure}[h!]
  ...
  \caption{Figure 4.5: Q-value spectrum.}
...
\begin{figure}[h!]
  ...
  \caption{Figure 4.8: Comparison of ToF spectra using $^7Li$, $^{12}C$, and phosphor targets. Notice the strength of the gamma peaks populated by beamstop-induced events relative to the peak populated by prompt target reactions.}
  \label{fig:9b Peak Comparison}
\end{figure}

To separate the neutron events from the $\gamma$ events, gates were applied to the PSD spectrum. The PSD spectra are all calibrated in units of keVee. Two threshold values were applied to the PSD spectra. In previous works, a threshold of 55 keVee and 85 keVee was applied to the 2"x2" and 4"x2" detectors. Due to the high beam-stop induced background, our minimum thresholds had to be increased significantly. The lowest value threshold value used is 200 keVee for both detector types. Excitation spectra where then extracte from the gated events. An excitation spectrum for a detector located at 123 degrees is shown below. 

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\textwidth]{tof_gated_123_labeled.png}
  \caption{Figure 4.9: A $^7Li$ neutron-gated ToF spectrum taken at 123 degrees. The labeled peaks represent expected flight times for neutron groups resulting from a populated state in $^9B$.}
...
\\ \\
A proposed experiment will be using the germanium detector array CLARION-2 in coincidence with CATRiNA to analyze heavier-mass nuclei.
